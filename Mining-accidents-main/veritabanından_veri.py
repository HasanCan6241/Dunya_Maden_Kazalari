...
for i in LANGUAGES.keys():
    a = 0
    komut_SELECT = "select * from urls_sehir where ulke_kod='{}'".format(i)
    cursor.execute(komut_SELECT)
    liste = cursor.fetchall()
    print()
    for i in liste:
        request=requests.get(i[0])
        print(i[0])
        b_source=BeautifulSoup(request.content,"html.parser")
        if(a==1):
            break
        try:
            for j in b_source.find_all('tr'):
                print((j.text.split("\n\n")) if(len(j.text.split("\n\n")[0])<50) else "\n")
            a=1
        except:
            pass
